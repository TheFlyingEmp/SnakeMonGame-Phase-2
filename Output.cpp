...
{
	///TODO:(done) implement the following function as described in Output.h file
...
void Output::DrawCardNumber(const CellPosition & cellPos, int cardNum) const
{
	...
	pWind->DrawRectangle(cellStartX, cellStartY, cellStartX + (UI.width / NumHorizontalCells), cellStartY + ((UI.height - UI.ToolBarHeight - UI.StatusBarHeight) / NumVerticalCells));

	pWind->GetIntegerSize(w, h, cardNum);

	///TODO:(done) Calculate the Width & Height of the integer "cardNum" if written using the Current Font
	...
	int x = cellStartX + UI.CellWidth - UI.LadderXOffset - w; // Before the End vertical line of a ladder 
	                                                              // (assuming the case where ladder is inside the cell)
	int y = cellStartY + (UI.CellHeight - h) / 2;	// in the vertical Middle of the cell


	///TODO:(done) Draw the Integer the representing the "cardNum" in the location (x,y)
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_DELETE_GAME_OBJECT] = "images\\Menu_Game_Object.jpg";
...
{
	///TODO:(done) Clear what was written on the toolbar
	...
	///TODO:(done) Calculate the Width and Height of the string if drawn using the current font 
	...
	///TODO:(done) Draw the string "info" in the specified location (x, y)
...
void Output::DrawCell(const CellPosition & cellPos, int cardNum) const
{
	...
	///TODO:(done) Draw the Cell Rectangle using the appropriate coordinates
	...
	///TODO:(done) Get the Width and Height of the Cell Number if written using the current font 
	//       (Use GetIntegerSize() window function) and set the "w" and "h" variables with its width and height
	...
	///TODO:(done) Draw the cell number in the x and y location
...
void Output::DrawPlayer(const CellPosition& cellPos, int playerNum, color playerColor) const
{
	...
	///TODO:(done) Validate the playerNum, if not valid return
	...
	///TODO:(done) Draw the player circle in center(x,y) and filled with the playerColor passed to the function
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	///TODO:(done) Validate the Cell Position (Must be Vertical Cells AND toCell is above fromCell, otherwise, Do NOT draw)

	if (toCell.VCell() >= fromCell.VCell()) return;
	else if (toCell.HCell() != fromCell.HCell()) return;
	...
	///TODO:(done) Set pen color and width using the appropriate parameters of UI_Info object (UI)

	pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);

	///TODO:(done) Draw The First Vertical Line (The Left Line) in the appropriate coordinates
...
///TODO:(done) Set pen color and width using the appropriate variables of UI_Info object (UI)

	pWind->SetPen(UI.LadderColor, UI.LadderlineWidth);

	///TODO:(done) Draw The Second Vertical Line (The Right Line) in the appropriate coordinates
	...
	///TODO:(done) Draw the cross horizontal lines of the ladder using the appropriate coordinates

	for (int i = 0; i < (fromCell.VCell() - toCell.VCell()); i++)
	{
		int yHline = (toCell.VCell() + i + 1) * ((UI.height - UI.ToolBarHeight - UI.StatusBarHeight) / NumVerticalCells) + UI.ToolBarHeight;
		pWind->DrawLine(x12, yHline, x34, yHline);
	}
}
...
void Output::DrawSnake(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	///TODO:(done) Validate the fromCell and toCell (Must be Vertical and toCell is below fromCell otherwise do NOT draw)
	...
	///TODO:(done) Set pen color and width from the appropriate variables of the UI_Info object (UI)

	pWind->SetPen(UI.SnakeColor, UI.SnakelineWidth);


	///TODO:(done) Draw the Line representing the Snake BOdy
	...
	///TODO:(done) Set the coordinates of the 4 points of the Polygon
	//       Check the snakes drawn in the project document and draw it the same way

	int& yCenter = y1;
	int yUp = yCenter - yChange;
	int yDown = yCenter + yChange;
	int& xCenter = x12;
	int xRight = xCenter + xChange;
	int xLeft = xCenter - xChange;

	///TODO:(done) Draw the Polygon (diamond) representing the Snake's Head
	//       Check the snakes drawn in the project document and draw it the same way

	int arrayX[] = {xCenter, xRight, xCenter, xLeft};
	int arrayY[] = {yUp, yCenter, yDown, yCenter};
	pWind->DrawPolygon(arrayX, arrayY, 4, FILLED);
}
